...
export function Activos() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      load();
    } catch { toast.error('Error guardando activo'); }
  };
  ...
  return (
    <div className="space-y-4 page-enter">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setModal(true)} className="btn-primary flex items-center gap-2"><i className="ti ti-plus text-sm"/> Agregar activo</button>
      </div>
      <div className="bg-g-800 rounded-2xl p-5 text-white">
        ...
        <p className="text-white/30 text-xs mt-1">{items.length} activo{items.length!==1?'s':''} registrado{items.length!==1?'s':''}</p>
      </div>
      <div className="grid grid-cols-2 gap-3">
        {items.length===0 && <div className="card col-span-2 p-12 text-center"><i className="ti ti-building-bank text-4xl text-g-200 block mb-2"/><p className="text-g-400 text-sm">No tienes activos registrados</p></div>}
        {items.map(a => {
          const ganancia = parseFloat(a.valor_actual)-parseFloat(a.valor_inicial);
          return (
            <div key={a.id} className="card p-4">
              <div className="flex items-start justify-between mb-3">
                <div><p className="font-medium text-g-900 text-sm">{a.nombre}</p><span className="badge-ok text-[10px]">{a.tipo}</span></div>
                <button onClick={() => del(a.id)} className="text-g-300 hover:text-red-500 transition-colors"><i className="ti ti-trash text-sm"/></button>
              </div>
              <p className="text-xl font-medium text-g-900">{fmt(a.valor_actual)}</p>
              <p className={`text-xs mt-0.5 ${ganancia>=0?'text-g-500':'text-red-500'}`}>{ganancia>=0?'+':''}{fmt(ganancia)} desde adquisición</p>
              {a.fecha_adquisicion && <p className="text-[11px] text-g-400 mt-2">Desde {fmtDate(a.fecha_adquisicion)}</p>}
            </div>
          );
        })}
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-g-100">
              <h3 className="font-medium text-g-900">Nuevo activo</h3>
            ...
            <form onSubmit={submit} className="p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div><label className="section-label block mb-1">Valor inicial</label><input type="number" className="input" placeholder="0" value={form.valor_inicial} onChange={set('valor_inicial')} required/></div>
                <div><label className="section-label block mb-1">Valor actual</label><input type="number" className="input" placeholder="0" value={form.valor_actual} onChange={set('valor_actual')}/></div>
              </div>
...
export function Deudas() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      load();
    } catch { toast.error('Error guardando deuda'); }
  };

  const pagar = async (d) => {
    const abono = prompt(`¿Cuánto quieres abonar a "${d.nombre}"?`);
    if (!abono) return;
  ...
  return (
    <div className="space-y-4 page-enter">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setModal(true)} className="btn-primary flex items-center gap-2"><i className="ti ti-plus text-sm"/> Agregar deuda</button>
      </div>
          ...
          return (
            <div key={d.id} className={`card p-4 ${!d.activa?'opacity-60':''}`}>
              <div className="flex items-start justify-between mb-2">
                <div>
                  <div className="flex items-center gap-2">
                    <p className="font-medium text-g-900 text-sm">{d.nombre}</p>
                ...
                <div className="flex gap-1">
                  {d.activa && <button onClick={() => pagar(d)} className="text-xs btn-secondary py-1 px-2">Abonar</button>}
              ...
              <div className="flex justify-between text-xs text-g-500 mb-1.5">
                <span>Pagado: {fmt(d.monto_pagado)}</span>
                <span>Pendiente: <span className="text-red-600 font-medium">{fmt(pendiente)}</span></span>
              </div>
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-g-100">
              <h3 className="font-medium text-g-900">Nueva deuda</h3>
              <button onClick={() => setModal(false)}><i className="ti ti-x text-g-400"/></button>
            </div>
            <form onSubmit={submit} className="p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div><label className="section-label block mb-1">Monto total</label><input type="number" className="input" placeholder="0" value={form.monto_total} onChange={set('monto_total')} required/></div>
...
export function Metas() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      load();
    } catch { toast.error('Error guardando meta'); }
  };

  const abonar = async (m) => {
    const abono = prompt(`¿Cuánto quieres aportar a "${m.nombre}"?`);
    if (!abono) return;
    ...
    toast.success(nuevo>=m.monto_objetivo?'¡Meta lograda! 🎉':'Aporte registrado');
    load();
  };
  ...
  return (
    <div className="space-y-4 page-enter">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setModal(true)} className="btn-primary flex items-center gap-2"><i className="ti ti-plus text-sm"/> Nueva meta</button>
      </div>
      <div className="grid grid-cols-2 gap-3">
        {items.length===0 && <div className="card col-span-2 p-12 text-center"><i className="ti ti-target text-4xl text-g-200 block mb-2"/><p className="text-g-400 text-sm">Aún no tienes metas. ¡Crea la primera!</p></div>}
        {items.map(m => {
          ...
          return (
            <div key={m.id} className={`card p-4 ${m.completada?'border-g-300':''}`}>
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2">
                  <div className="w-9 h-9 rounded-xl bg-g-50 flex items-center justify-center"><i className={`ti ${m.icono} text-g-600`}/></div>
                  <div><p className="font-medium text-g-900 text-sm">{m.nombre}</p>{m.completada && <span className="badge-ok text-[10px]">Completada 🎉</span>}</div>
                </div>
                <button onClick={() => del(m.id)} className="text-g-300 hover:text-red-500 p-1"><i className="ti ti-trash text-sm"/></button>
              </div>
              <div className="flex justify-between text-xs text-g-500 mb-1.5"><span>{fmt(m.monto_actual)}</span><span>{fmt(m.monto_objetivo)}</span></div>
              <div className="h-2.5 bg-g-100 rounded-full overflow-hidden"><div className="h-full bg-g-400 rounded-full" style={{width:`${pct}%`}}/></div>
              <div className="flex items-center justify-between mt-2">
                <p className="text-[11px] text-g-400">{pct}% logrado</p>
                {!m.completada && <button onClick={() => abonar(m)} className="text-xs text-g-600 hover:text-g-800 underline underline-offset-2">Aportar</button>}
              </div>
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-g-100">
              <h3 className="font-medium text-g-900">Nueva meta</h3>
              <button onClick={() => setModal(false)}><i className="ti ti-x text-g-400"/></button>
            </div>
            <form onSubmit={submit} className="p-5 space-y-3">
              ...
              <input className="input" placeholder="Descripción (opcional)" value={form.descripcion} onChange={set('descripcion')}/>
              <div className="grid grid-cols-2 gap-2">
                <div><label className="section-label block mb-1">Monto objetivo</label><input type="number" className="input" placeholder="0" value={form.monto_objetivo} onChange={set('monto_objetivo')} required/></div>
                <div><label className="section-label block mb-1">Fecha límite</label><input type="date" className="input" value={form.fecha_limite} onChange={set('fecha_limite')}/></div>
              </div>
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {ICONOS_META.map(ic => (
                    <button key={ic} type="button" onClick={() => setForm(f => ({...f,icono:ic}))}
                      className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${form.icono===ic?'bg-g-700 text-white':'bg-g-50 text-g-500 hover:bg-g-100'}`}>
